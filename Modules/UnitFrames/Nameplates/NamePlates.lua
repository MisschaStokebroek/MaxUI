...
------------------------------------------------------------------------------------------
	...
	if C["NamePlates"]["Debuffs"] == true then
		if C["NamePlates"]["DebuffsOnTop"] == true then
			...
			Debuffs:SetPoint("BOTTOMLEFT", self, "TOPLEFT", 0, 22)
		end
...
------------------------------------------------------------------------------------------
	if C["UnitFrames"]["Style"]["Value"] == "MaxUI" then
		...
		if C.NamePlates.NameplateCastBar then
			...
			Castbar.Button:SetPoint("TOPRIGHT", self, "TOPLEFT", -6, 2)
		...
		-- questicon
		if T.Retail and C.NamePlates.QuestIcon then
			QuestIcon:SetTexture([[Interface\AddOns\MaxUI\Medias\microbar\quest.tga]])
		end
	
		-- classicon
		if C.NamePlates.ClassIcon then
			ClassIcon:SetSize(self:GetHeight() + 6, self:GetHeight() + 6)
			ClassIcon:ClearAllPoints()
			ClassIcon:SetPoint("BOTTOMRIGHT", Name, "BOTTOMLEFT", -4, 0)
		end
